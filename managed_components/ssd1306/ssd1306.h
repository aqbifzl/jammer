...
#define OLED_DRAW_ALL                                                          \
  (OLED_DRAW_UPPER_RIGHT | OLED_DRAW_UPPER_LEFT | OLED_DRAW_LOWER_RIGHT |      \
   OLED_DRAW_LOWER_LEFT)
...
void ssd1306_display_image(SSD1306_t *dev, int page, int seg,
                           const uint8_t *images, int width);
void ssd1306_display_text(SSD1306_t *dev, int page, const char *text,
                          int text_len, bool invert);
void ssd1306_display_text_box1(SSD1306_t *dev, int page, int seg,
                               const char *text, int box_width, int text_len,
                               bool invert, int delay);
void ssd1306_display_text_box2(SSD1306_t *dev, int page, int seg,
                               const char *text, int box_width, int text_len,
                               bool invert, int delay);
void ssd1306_display_text_x3(SSD1306_t *dev, int page, const char *text,
                             int text_len, bool invert);
void ssd1306_clear_screen(SSD1306_t *dev, bool invert);
void ssd1306_clear_line(SSD1306_t *dev, int page, bool invert);
void ssd1306_contrast(SSD1306_t *dev, int contrast);
void ssd1306_software_scroll(SSD1306_t *dev, int start, int end);
void ssd1306_scroll_text(SSD1306_t *dev, const char *text, int text_len,
                         bool invert);
void ssd1306_scroll_clear(SSD1306_t *dev);
void ssd1306_hardware_scroll(SSD1306_t *dev, ssd1306_scroll_type_t scroll);
void ssd1306_wrap_arround(SSD1306_t *dev, ssd1306_scroll_type_t scroll,
                          int start, int end, int8_t delay);
void _ssd1306_bitmaps(SSD1306_t *dev, int xpos, int ypos, const uint8_t *bitmap,
                      int width, int height, bool invert);
void ssd1306_bitmaps(SSD1306_t *dev, int xpos, int ypos, const uint8_t *bitmap,
                     int width, int height, bool invert);
...
void _ssd1306_circle(SSD1306_t *dev, int x0, int y0, int r, unsigned int opt,
                     bool invert);
void _ssd1306_disc(SSD1306_t *dev, int x0, int y0, int r, unsigned int opt,
                   bool invert);
...
void ssd1306_display_rotate_text(SSD1306_t *dev, int seg, const char *text,
                                 int text_len, bool invert);
...
void i2c_display_image(SSD1306_t *dev, int page, int seg, const uint8_t *images,
                       int width);
void i2c_contrast(SSD1306_t *dev, int contrast);
void i2c_hardware_scroll(SSD1306_t *dev, ssd1306_scroll_type_t scroll);

void spi_clock_speed(int speed);
void spi_master_init(SSD1306_t *dev, int16_t mosi, int16_t sclk, int16_t cs,
                     int16_t dc, int16_t reset);
...
bool spi_master_write_byte(spi_device_handle_t SPIHandle, const uint8_t *Data,
                           size_t DataLength);
bool spi_master_write_commands(SSD1306_t *dev, const uint8_t *Commands,
                               size_t DataLength);
...
void spi_display_image(SSD1306_t *dev, int page, int seg, const uint8_t *images,
                       int width);
